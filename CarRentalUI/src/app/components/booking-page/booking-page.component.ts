...
export class BookingPageComponent implements OnInit {
  ...
  constructor(private service: ServicesService, private router: ActivatedRoute,private Router : Router) {
  ...
  Submit() {
    ...
    if (startDate <= today) {
      this.formError = 'Start date must be greater than today.';
    } else if (endDate < startDate) {
      this.formError = 'End date must be greater than start date.';
    } else {
      const carId = this.id;
